...
int main(void)
{
	...
	while(1==1)
	{
		if(RX_DoneIndex)
		{
			int idx = RX_DoneIndex & 1;
			
			printf("RX_Buffer[%d]: ", idx);
			for(int i = 0; i < 4; i++)
				printf("%04X, ", RX_Buffer[idx][i]);
			printf("\n\n");
...
//	PORT_Init(PORTB, PIN3, PORTB_PIN3_SPI1_MISO, 0);	// I2S_DO
	...
	DMA_initStruct.Handshake = DMA_CH3_SPI1RX;
	DMA_initStruct.Priority = DMA_PRI_LOW;
...
{
	/* Master TX */
	if(DMA_CH_INTStat(DMA_CH0, DMA_IT_SRCSG_HALF))
	{
		DMA_CH_INTClr(DMA_CH0, DMA_IT_SRCSG_HALF);
		
		TX_DoneIndex = (1 << 1) | 0;
	}
	else if(DMA_CH_INTStat(DMA_CH0, DMA_IT_SRCSG_DONE))
	{
		DMA_CH_INTClr(DMA_CH0, DMA_IT_SRCSG_DONE);
		
		TX_DoneIndex = (1 << 1) | 1;
	}
	
	/* Slave RX */
	if(DMA_CH_INTStat(DMA_CH3, DMA_IT_DSTSG_HALF))
	{
		DMA_CH_INTClr(DMA_CH3, DMA_IT_DSTSG_HALF);
		
		RX_DoneIndex = (1 << 1) | 0;
		
		GPIO_InvBit(GPIOA, PIN5);
	}
	else if(DMA_CH_INTStat(DMA_CH3, DMA_IT_DSTSG_DONE))
	{
		DMA_CH_INTClr(DMA_CH3, DMA_IT_DSTSG_DONE);
