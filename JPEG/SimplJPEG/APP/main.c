...
uint8_t  *jpeg_image_sdr = (uint8_t *)(PSRAMM_BASE + 0x100000);
...
int main(void)
{
	...
	memcpy(jpeg_image_sdr, jpeg_image2, sizeof(jpeg_image2));
	jfif_parse(jpeg_image_sdr, sizeof(jpeg_image2), &jfif_info);
	
	jpeg_outset.format = JPEG_OUT_RGB565;
	jpeg_outset.dither = 0;
	jpeg_outset.RGBAddr = PSRAMM_BASE;
